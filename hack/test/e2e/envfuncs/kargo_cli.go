...
func LoadKargoClient(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
	if kargoConfig, ok := ctx.Value("kargo_config").(config.CLIConfig); ok {
		...
		if err != nil {
			return ctx, fmt.Errorf("error loading kargo client: %v", err)
		}
		return context.WithValue(ctx, "kargo_cli", *kargoClient), nil
	}
	fmt.Fprintf(os.Stderr, "error getting kargo_config from the context %v", ctx)
	return ctx, nil
}

func withKargoConfig(ctx context.Context, kargoConfig config.CLIConfig) context.Context {
	return context.WithValue(ctx, "kargo_config", kargoConfig)
}
